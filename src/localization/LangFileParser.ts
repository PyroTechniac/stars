...
export class LangFileParser {
    ...
    public static parseFile(langName: string, filePath: string, fileContents: string): Language {
        ...
        if (!blocks) throw new Error(`The given Lang file contains no valid localization strings: ${filePath}`);
        ...
        for (const block of blocks) {
            ...
            if (LangFileParser._scriptTemplates.test(value)) {
                ...
                for (const script in templates) {
                    ...
                    scriptData.match(LangFileParser._scriptTemplate)![1] || scriptData.match(LangFileParser._scriptTemplate)![2];
